<?php
                require 'topo.php';
		$registros = $db->listar('vw_disco_usu','nome,mb');
		$registros2 = $db->listar('vw_disco_servico','usuario,servico,mb');
  ...
  foreach ($registros2 as $registro){
                $label2[]  = $registro['servico'];
                $values2[]  = $registro['usuario'];
                $tamanho2[]  = $registro['mb'];
   }

	$label2  = '\''.implode('\',\'', $label2) . '\'';
	$values2 = '\''.implode('\',\'', $values2) . '\'';
	$tamanho2 = implode(',', $tamanho2);
